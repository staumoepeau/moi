...
from  moi.moi_hr.doctype.moi_hr_file.exceptions import (
...
class MOIHRFile(Document):
	...
	def get_full_path(self):
		"""Override to look into the media/forms structure"""
		file_path = self.file_url or self.file_name

		# If the URL contains our custom path, resolve it correctly
		if "/files/media/forms/" in file_path:
			# Strip leading slash and 'files/' to get the relative path inside 'public'
			rel_path = file_path.lstrip('/').replace('files/', '', 1)
			return frappe.get_site_path("public", "files", rel_path)

		# Fallback to standard logic for everything else
	...
	def save_file_on_filesystem(self):
		"""Force the file to save into the media/forms/{category} path"""
		safe_file_name = re.sub(r"[/\\%?#]", "_", self.file_name)
		
		# Get the category from the form dict or the document itself
		category = frappe.flags.hr_category or "GENERAL"
		
		# Define the organized path
		folder_rel_path = os.path.join("media", "forms", category)
		full_path_dir = frappe.get_site_path("public", "files", folder_rel_path)
		
		if not os.path.exists(full_path_dir):
			os.makedirs(full_path_dir, mode=0o755)
			
		self.file_url = f"/files/media/forms/{category}/{safe_file_name}"
		
		# Now write the file to the path we just defined
		dest_path = os.path.join(full_path_dir, safe_file_name)
		...
		return {"file_name": safe_file_name, "file_url": self.file_url}
